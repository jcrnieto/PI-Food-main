...
const getAllRecipes = async (req, res, next) => {
    ...
    try{
   const apiInfo= recetas; //await axios.get(`https://api.spoonacular.com/recipes/complexSearch?apiKey=892adbcafb65413ca6db50b72545910c&addRecipeInformation=true&number=100`)
...
const getIdRecipes = async (req, res, next) => {
    ...
    try{
    const apiInfo= recetas; //await axios.get(`https://api.spoonacular.com/recipes/complexSearch?apiKey=4f2296d86cd45c9a4707181bef99c31&addRecipeInformation=true&number=100`) 
    //const infoId= apiInfo.data.results.map(el=>{
    const infoId= apiInfo.results.map(el=>{
        return{
            id: el.id,
            image: el.image,
            title: el.title,
...
const getAllTypes = async (req, res, next) => {
  ...
  // const diets = apiInfo.data.results.map(el=>el.diets)
   const diets = apiInfo.results.map(el=>el.diets)
